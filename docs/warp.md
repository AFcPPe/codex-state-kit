...
如果当前网络已经通过路由器、操作系统或企业出口统一提供外网访问，可以在「Token 获取代理」中选择「不使用代理」。此模式会让 Token 获取请求直连目标地址，并显式关闭应用对 `HTTP_PROXY` / `HTTPS_PROXY` 等代理环境变量的继承；它不会改变独立的「上游转发代理」设置。

与手动代理不同，直连模式不需要填写代理 URL，也不会因为代理为空而持续重试等待。
