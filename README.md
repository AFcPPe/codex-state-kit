...
在 Token 状态卡片中，点击模型旁的「禁用获取」可停止该模型的自动预取和手动刷新（例如不希望持续探测的 `codex-auto-review`）。设置会保存到本地，重启、切换账号或再次发现该模型都不会自动解除禁用；禁用的模型会一直显示，点击「解除禁用」恢复获取。其他模型不受影响，已有缓存仍可复用。

使用「无票等待」策略时，若该模型已禁用且无可用缓存，请求会明确报错而非无限等待；可解除禁用或更改无票策略。
