...
fn write_account(home: &Path, account: &str) {
    let claims = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(
        serde_json::json!({"chatgpt_account_id":account,"email":format!("{account}@example.com")}).to_string()
    );
...
fn request(account: Option<&str>, state: &str) -> Request<Body> {
    let mut request = Request::builder().method("POST").uri("/responses")
    ...
    request.body(Body::from(r#"{"model":"switch-model"}"#)).unwrap()
}
...
async fn account_switch_preserves_request_identity_and_rejects_old_tickets() {
    ...
    tokio::time::timeout(Duration::from_secs(15), async {
        let home = tempfile::tempdir().unwrap();
        let (send, mut requests) = tokio::sync::mpsc::channel::<(HeaderMap, oneshot::Sender<Response>)>(8);
        let router = axum::Router::new().route("/responses", post(move |headers: HeaderMap, _body: axum::body::Bytes| {
            let send = send.clone();
            async move {
        ...
        let app = Arc::new(App::new(Settings {
            upstream: endpoint.clone(), upstream_proxy: String::new(),
            outbound_proxy: endpoint, outbound_mode: OutboundMode::Manual,
            codex_home: home.path().display().to_string(), ..Settings::default()
        }).unwrap());
        let token_a = ticket(1);
        let token_b = ticket(2);
        app.sync_logged_in_account().await;
        assert!(app.turn_state.lock().await.capture("switch-model", &token_a, "test"));

        // A remains in flight while B logs in; its log identity must stay A.
        let pending = tokio::spawn(proxy_http(app.clone(), request(Some("account-a"), &token_a)));
        ...
        reply.send(Response::new(Body::from_stream(futures_util::stream::once(async move {
            wait_a.await.unwrap();
        ...
        for client in [Some("account-a"), Some("account-b"), None] {
            ...
            if client != Some("account-b") { assert!(!headers.contains_key(turn_state::HEADER_NAME)); }
            else { assert_eq!(headers[turn_state::HEADER_NAME], token_a); } // Caller-supplied same-account state is opaque.
            reply.send(Response::new(Body::from("done-b"))).unwrap();
            axum::body::to_bytes(pending.await.unwrap().into_body(), 1024).await.unwrap();
            let entry = app.logs.lock().await.back().cloned().unwrap();
            assert_eq!(entry.account_id.as_deref(), Some("account-b"));
            assert_eq!(entry.account_email.as_deref(), Some("account-b@example.com"));
            if client != Some("account-b") { assert_eq!(entry.turn_state_action, "removed_account_mismatch"); }
        }
        app.sync_logged_in_account().await;
        assert!(app.turn_state.lock().await.peek_for_model("switch-model").is_none());
        assert!(app.turn_state.lock().await.capture("switch-model", &token_b, "test"));
        let pending = tokio::spawn(proxy_http(app.clone(), request(Some("account-a"), &token_a)));
        let (headers, reply) = requests.recv().await.unwrap();
        ...
        axum::body::to_bytes(response_a.into_body(), 1024).await.unwrap();
        let logs = app.logs.lock().await;
        assert_eq!(logs.front().unwrap().account_id.as_deref(), Some("account-a"));
        assert_eq!(logs.front().unwrap().account_email.as_deref(), Some("account-a@example.com"));
        ...
        write_account(home.path(), "account-b");
        let response = Response::builder().header(turn_state::HEADER_NAME, &token_a).body(Body::empty()).unwrap();
        reply.send(response).unwrap();
        assert_eq!(pending.await.unwrap().unwrap_err().retry, FetchRetryClass::Stale);
        assert!(app.turn_state.lock().await.peek_for_model("switch-model").is_none());
        let entry = app.logs.lock().await.back().cloned().unwrap();
        assert_eq!(entry.account_id.as_deref(), Some("account-a"));
        assert_eq!(entry.account_email.as_deref(), Some("account-a@example.com"));
